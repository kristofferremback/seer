...
Use 1 to 16 ordered groups. Group `id` values are unique slugs. Every group title is at most 60 characters. Categories are `Contract`, `Code`, `Tests`, `Test fixtures`, `Docs`, or `Generated`. Importance and complexity are `low`, `medium`, or `high`. `examples` is required on every group, and `[]` is valid.
