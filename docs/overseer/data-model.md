...
How the delta renders is part of the design, not a renderer whim. Revised prose stays clean under a short `(Edited)` control. Opening it swaps a word-level redline into the same place: prior words precede their replacements and current words carry the insertion mark. A one-line field inside an existing row borrows that row's disclosure and shows the same redline only while open. Density never switches the page to a second diff grammar. New and removed rows keep neutral headings while collapsed, with a small state label as the change indication; opening them opts into the green or red treatment and the former content. Removed rows remain near the next surviving section from their base ordering rather than collecting at the end. Author intent, the witness summary and code design are not exempt: authored text diffs wherever it appears. The page states which base version its marks are measured against, and row-level marks exist only as the sum of their spans, so a mark can never claim a change the text does not show.
