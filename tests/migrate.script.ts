...
const V21_TABLES = [
  "review_thread_scopes", "review_threads", "review_thread_anchors", "review_thread_entries", "review_thread_idempotency",
  "review_github_threads", "review_github_thread_observations", "review_github_comments", "review_github_comment_observations",
  "review_github_reviews", "review_github_review_observations", "review_conversation_imports", "review_conversation_refresh_idempotency",
  "share_capability_local_threads", "share_capability_github_threads", "share_capability_github_comments", "share_capability_github_reviews",
];
const V23_TABLES = [
  "review_github_projection_preferences", "github_graphql_rate_limits",
  "review_github_viewed_jobs", "review_github_viewed_ownership", "review_github_viewed_credential_substitutions",
  "review_github_submissions", "review_github_submission_rebinds", "review_github_resolution_requests", "review_local_github_threads",
  "review_local_github_message_links", "review_local_github_resolution_links",
];
const V24_TABLES = [
  "review_legacy_successions", "review_legacy_succession_members", "review_legacy_succession_idempotency",
];
const V22_THREAD_OBSERVATIONS_RESTORE = `
  CREATE TABLE review_github_thread_observations_v22 (
    id TEXT PRIMARY KEY,
  ...
  INSERT INTO review_github_thread_observations_v22 SELECT * FROM review_github_thread_observations;
  DROP TABLE review_github_thread_observations;
  ALTER TABLE review_github_thread_observations_v22 RENAME TO review_github_thread_observations;
...
const V22_DOMAIN_TABLES = [
  "review_revision_acknowledgements", "review_revision_acknowledgement_carries", "review_revision_acknowledgement_boundaries",
  "review_revision_item_equivalences", "review_revision_judgments", "review_revision_judgment_items",
  "review_stack_judgments", "review_stack_judgment_items",
];
// Historical scenarios use this as "everything added after v21". Keep later tables in
// that aggregate so every old-image restore proves they disappear too.
const V22_TABLES = [...V22_DOMAIN_TABLES, ...V23_TABLES, ...V24_TABLES];
...
if (SCENARIO === "v0") {
  ...
  assert(uv === 24, `user_version should be 24, got ${uv}`);
...
if (SCENARIO === "v11") {
  ...
  migrate();
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 24, `v11 database reaches 24, got ${uv}`);
  ...
  for (const table of [...V16_TABLES, ...V17_TABLES, ...V18_TABLES, ...V19_TABLES, ...V20_TABLES, ...V21_TABLES, ...V22_TABLES]) assert(!!db.query("SELECT name FROM sqlite_master WHERE type = 'table' AND name = ?").get(table), `table ${table} exists after the v16 through v19 migrations`);
...
if (SCENARIO === "v12") {
  ...
  migrate();
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 24, `v12 database reaches 24, got ${uv}`);
...
if (SCENARIO === "fresh") {
  ...
  for (const table of [...V3_TABLES, ...V4_TABLES, ...V5_TABLES, ...V6_TABLES, ...V12_TABLES, ...V13_TABLES, ...V14_TABLES, ...V15_TABLES, ...V16_TABLES, ...V17_TABLES, ...V18_TABLES, ...V19_TABLES, ...V20_TABLES, ...V21_TABLES, ...V22_TABLES]) {
...
if (SCENARIO === "v2") {
  ...
  assert(uv2 === 24, `user_version stays 24 after re-run, got ${uv2}`);
  ...
  try {
    ...
    threw = true;
    assert(/user_version 25/.test((err as Error).message), `actionable message, got: ${(err as Error).message}`);
  }
...
if (SCENARIO === "v5stops") {
  ...
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 24, `ordinary boot reaches 24, got ${uv}`);
  const still = db
...
if (SCENARIO === "v5dropafterboot") {
  ...
  assert(first === 24, `ordinary boot reaches 24, got ${first}`);
  assert(
    !!db
      .query("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'review_freshness'")
      .get(),
  ...
  assert(after === 24, `and the version is untouched by it, got ${after}`);
...
if (SCENARIO === "v6ambiguous") {
  ...
  db.exec(`
    ...
    ${[...V20_TABLES, ...V21_TABLES, ...V22_TABLES].map((table) => `DROP TABLE ${table};`).join("\n")}
...
if (SCENARIO === "v13") {
  ...
  const { db } = await import("../src/db");
  migrate();
  db.exec([...V20_TABLES, ...V21_TABLES, ...V22_TABLES].map((table) => `DROP TABLE ${table};`).join(" ") + " DROP TABLE stage_change_reads; PRAGMA user_version = 13;");
  db.run(
...
if (SCENARIO === "v14") {
  ...
  db.exec([...V15_TABLES, ...V16_TABLES, ...V17_TABLES, ...V20_TABLES, ...V21_TABLES, ...V22_TABLES].map((table) => `DROP TABLE ${table};`).join(" ") + " PRAGMA user_version = 14;");
  ...
  migrate();
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 24, `a populated v14 database reaches 24, got ${uv}`);
...
if (SCENARIO === "v15") {
  ...
  migrate();
  db.exec([...V16_TABLES, ...V17_TABLES, ...V20_TABLES, ...V21_TABLES, ...V22_TABLES].map((table) => `DROP TABLE ${table};`).join(" ") + " PRAGMA user_version = 15;");
  ...
  migrate();
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 24, `a populated v15 database reaches 24, got ${uv}`);
...
if (SCENARIO === "v16") {
  ...
  migrate();
  db.exec([...V17_TABLES, ...V18_TABLES, ...V19_TABLES, ...V20_TABLES, ...V21_TABLES, ...V22_TABLES].map((table) => `DROP TABLE ${table};`).join(" ") + " PRAGMA user_version = 16;");
  ...
  migrate();
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 24, `a populated v16 database reaches 24, got ${uv}`);
  ...
  for (const table of [...V17_TABLES, ...V18_TABLES, ...V19_TABLES, ...V20_TABLES, ...V21_TABLES, ...V22_TABLES]) {
...
if (SCENARIO === "v17") {
  ...
  migrate();
  db.exec([...V18_TABLES, ...V19_TABLES, ...V20_TABLES, ...V21_TABLES, ...V22_TABLES].map((table) => `DROP TABLE ${table};`).join(" ") + " PRAGMA user_version = 17;");
  db.run(
  ...
  migrate();
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 24, `a populated v17 database reaches 24, got ${uv}`);
  ...
  for (const table of [...V18_TABLES, ...V19_TABLES, ...V20_TABLES, ...V21_TABLES, ...V22_TABLES]) {
...
if (SCENARIO === "v18") {
  ...
  migrate();
  db.exec([...V19_TABLES, ...V20_TABLES, ...V21_TABLES, ...V22_TABLES].map((table) => `DROP TABLE ${table};`).join(" ") + " PRAGMA user_version = 18;");
  ...
  db.run("INSERT INTO review_revision_movements (revision_id, workspace_id, lineage_id, previous_revision_id, unchanged, revised, new, removed, computed_at) VALUES (?, ?, ?, ?, 1, 0, 0, 0, 3)", ["rvr_0000000019", "ws_0000000000", "rln_0000000018", "rvr_0000000018"]);
  ...
  migrate();
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 24, `a populated v18 database reaches 24, got ${uv}`);
...
if (SCENARIO === "v19") {
  ...
  migrate();
  db.exec([...V21_TABLES, ...V22_TABLES].map((table) => `DROP TABLE ${table};`).join(" "));
  db.exec(`
  ...
  migrate();
  assert((db.query("PRAGMA user_version").get() as { user_version: number }).user_version === 24, "v19 reaches v24");
  ...
  try {
    ...
    v19Refused = true;
    const message = (err as Error).message;
    assert(/user_version 24/.test(message) && /0 through 19/.test(message), `the v19 refusal names the version and range, got: ${message}`);
  }
...
// The v21 additive conversation migration over populated task-8 capability authority.
if (SCENARIO === "v20") {
  process.env.AUTH_DISABLED = "true";
  delete process.env.ALLOWED_EMAILS;
  const { migrate, assertDatabaseVersionSupported } = await import("../src/migrate");
  const { db } = await import("../src/db");
  const { Database: Sqlite } = await import("bun:sqlite");
  const { rmSync, writeFileSync: writeBackup } = await import("node:fs");
  migrate();
  db.exec([...V21_TABLES, ...V22_TABLES].map((table) => `DROP TABLE ${table};`).join(" "));
  db.exec(`
    ALTER TABLE share_document_capabilities RENAME TO share_document_capabilities_v21;
    CREATE TABLE share_document_capabilities (
      share_id TEXT PRIMARY KEY, workspace_id TEXT NOT NULL,
      document_kind TEXT NOT NULL CHECK (document_kind IN ('review_revision','review_account','stack_manifest','stack_account')),
      document_id TEXT NOT NULL, created_at INTEGER NOT NULL,
      UNIQUE (share_id, workspace_id, document_kind, document_id)
    );
    INSERT INTO share_document_capabilities (share_id, workspace_id, document_kind, document_id, created_at)
      SELECT share_id, workspace_id, document_kind, document_id, created_at FROM share_document_capabilities_v21;
    DROP TABLE share_document_capabilities_v21;
    CREATE INDEX idx_share_document_capabilities_document ON share_document_capabilities (workspace_id, document_kind, document_id);
    PRAGMA user_version = 20;
  `);
  const kinds = ["review_revision", "review_account", "stack_manifest", "stack_account"];
  kinds.forEach((kind, index) => {
    const id = `shr_00000002${index.toString().padStart(2, "0")}`;
    db.run("INSERT INTO shares (id, workspace_id, kind, target, label, token_hash, created_by, created_at) VALUES (?, 'ws_0000000000', ?, ?, ?, ?, 'usr_0000000000', ?)", [id, kind.startsWith("stack") ? "stack_document" : "review_document", `${kind.startsWith("stack") ? kind === "stack_manifest" ? "rsm" : "rsa" : kind === "review_revision" ? "rvr" : "rac"}_0000000200`, kind, `hash-${kind}`, index + 1]);
    db.run("INSERT INTO share_document_capabilities VALUES (?, 'ws_0000000000', ?, ?, ?)", [id, kind, `${kind.startsWith("stack") ? kind === "stack_manifest" ? "rsm" : "rsa" : kind === "review_revision" ? "rvr" : "rac"}_0000000200`, index + 1]);
    db.run("INSERT INTO share_capability_files VALUES (?, 'ws_0000000000', 1, 'rvr_0000000200', 'stg_0000000200', ?, 1)", [id, `stf_00000002${index.toString().padStart(2, "0")}`]);
    db.run("INSERT INTO share_capability_items VALUES (?, 'ws_0000000000', 1, 'rvr_0000000200', 'file', ?, 1)", [id, `stf_00000002${index.toString().padStart(2, "0")}`]);
  });
  db.run("INSERT INTO review_revisions (id, workspace_id, lineage_id, slug, revision, capture_id, schema_version, doc, digest, created_at) VALUES ('rvr_0000000200', 'ws_0000000000', 'rln_0000000200', 'v20-review', 1, 'stg_0000000200', 1, ?, 'revision-v20', 1)", ['{"v20":"revision"}']);
  db.run("INSERT INTO review_accounts (id, workspace_id, lineage_id, revision_id, revision, slug, version, schema_version, doc, digest, witness_user_id, witness_key_id, created_at) VALUES ('rac_0000000200', 'ws_0000000000', 'rln_0000000200', 'rvr_0000000200', 1, 'v20-review', 1, 1, ?, 'account-v20', 'usr_0000000000', 'key_0000000000', 1)", ['{"v20":"account"}']);
  db.run("INSERT INTO review_stack_manifests (id, stack_id, workspace_id, slug, version, predecessor_version, reason, schema_version, doc, digest, created_at) VALUES ('rsm_0000000200', 'rsk_0000000200', 'ws_0000000000', 'v20-stack', 1, 0, 'created', 1, ?, 'manifest-v20', 1)", ['{"v20":"manifest"}']);
  db.run("INSERT INTO review_stack_accounts (id, stack_id, manifest_id, workspace_id, slug, version, schema_version, doc, digest, witness_user_id, witness_key_id, created_at) VALUES ('rsa_0000000200', 'rsk_0000000200', 'rsm_0000000200', 'ws_0000000000', 'v20-stack', 1, 1, ?, 'stack-account-v20', 'usr_0000000000', 'key_0000000000', 1)", ['{"v20":"stack-account"}']);
  db.run("INSERT INTO review_revision_change_reads VALUES ('ws_0000000000', 'rvr_0000000200', 'usr_0000000000', ?, 1)", [`chg_${"c".repeat(64)}`]);
  db.run("INSERT INTO project_review_lineages VALUES ('prj_0000000200', 'ws_0000000000', 'v20-review', 1)");
  db.run("INSERT INTO project_review_stacks VALUES ('prj_0000000200', 'ws_0000000000', 'v20-stack', 1)");
  const beforeScopes = db.query("SELECT * FROM share_document_capabilities ORDER BY share_id").all();
  const beforeShares = db.query("SELECT * FROM shares WHERE id LIKE 'shr_00000002%' ORDER BY id").all();
  const beforeDocuments = db.query("SELECT doc, digest FROM review_revisions WHERE id = 'rvr_0000000200' UNION ALL SELECT doc, digest FROM review_accounts WHERE id = 'rac_0000000200' UNION ALL SELECT doc, digest FROM review_stack_manifests WHERE id = 'rsm_0000000200' UNION ALL SELECT doc, digest FROM review_stack_accounts WHERE id = 'rsa_0000000200'").all();
  const backup = db.serialize();
  migrate();
  assert((db.query("PRAGMA user_version").get() as { user_version: number }).user_version === 24, "v20 reaches v24");
  assert(JSON.stringify(db.query("SELECT share_id, workspace_id, document_kind, document_id, created_at FROM share_document_capabilities ORDER BY share_id").all()) === JSON.stringify(beforeScopes), "every v20 capability scope column survives");
  assert(JSON.stringify(db.query("SELECT * FROM shares WHERE id LIKE 'shr_00000002%' ORDER BY id").all()) === JSON.stringify(beforeShares), "every v20 share byte survives");
  assert(JSON.stringify(db.query("SELECT doc, digest FROM review_revisions WHERE id = 'rvr_0000000200' UNION ALL SELECT doc, digest FROM review_accounts WHERE id = 'rac_0000000200' UNION ALL SELECT doc, digest FROM review_stack_manifests WHERE id = 'rsm_0000000200' UNION ALL SELECT doc, digest FROM review_stack_accounts WHERE id = 'rsa_0000000200'").all()) === JSON.stringify(beforeDocuments), "every immutable document byte survives");
  assert(!!db.query("SELECT 1 FROM review_revision_change_reads WHERE revision_id = 'rvr_0000000200'").get(), "revision read survives");
  assert(!!db.query("SELECT 1 FROM project_review_lineages WHERE slug = 'v20-review'").get() && !!db.query("SELECT 1 FROM project_review_stacks WHERE slug = 'v20-stack'").get(), "Project joins survive");
  assert((db.query<{ n: number }, []>("SELECT COUNT(*) AS n FROM share_document_capabilities WHERE conversation_scope = 'none'").get()!.n) === 4, "all existing capabilities default to conversation none");
  for (const table of [...V21_TABLES, ...V22_TABLES]) assert((db.query(`SELECT COUNT(*) AS n FROM ${table}`).get() as { n: number }).n === 0, `${table} starts empty`);
  const scopeColumns = db.query<{ name: string }, []>("PRAGMA table_info(review_thread_scopes)").all().map((row) => row.name);
  assert(scopeColumns.includes("document_id") && !scopeColumns.includes("scope_id"), "local budgets key the immutable document");
  for (const table of ["share_capability_local_threads", "share_capability_github_threads", "share_capability_github_comments", "share_capability_github_reviews"]) {
    assert(db.query<{ name: string }, []>(`PRAGMA table_info(${table})`).all().some((row) => row.name === "workspace_id"), `${table} carries workspace containment`);
  }
  const commentSchema = db.query<{ sql: string }, [string]>("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ?").get("review_github_comments")!.sql;
  const reviewSchema = db.query<{ sql: string }, [string]>("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = ?").get("review_github_reviews")!.sql;
  assert(commentSchema.includes("workspace_id, thread_id, github_database_id") && reviewSchema.includes("workspace_id, lineage_id, github_database_id"), "imported identities include their thread or lineage boundary");
  let refused = false;
  try { assertDatabaseVersionSupported(20); } catch { refused = true; }
  assert(refused, "the v20 maximum refuses v24");
  db.close();
  for (const suffix of ["", "-wal", "-shm"]) rmSync(join(dataDir, `seer.db${suffix}`), { force: true });
  writeBackup(join(dataDir, "seer.db"), backup);
  const restored = new Sqlite(join(dataDir, "seer.db"), { readonly: true });
  assert((restored.query("PRAGMA user_version").get() as { user_version: number }).user_version === 20, "restore reopens under v20");
  assert(!restored.query<{ name: string }, []>("PRAGMA table_info(share_document_capabilities)").all().some((row) => row.name === "conversation_scope"), "v20 restore has no conversation column");
  restored.close();
  console.log("migrate v20: all assertions passed");
  process.exit(0);
}

// The v22 additive acknowledgement and judgment migration over every exact document and
// conversation family task 10 must leave untouched. Rollback restores the serialized v21
// database; a v21 binary never opens the migrated shape.
if (SCENARIO === "v21") {
  process.env.AUTH_DISABLED = "true";
  delete process.env.ALLOWED_EMAILS;
  const { migrate, assertDatabaseVersionSupported } = await import("../src/migrate");
  ...
  const { rmSync, writeFileSync: writeBackup } = await import("node:fs");
  migrate();
  db.exec(V22_TABLES.map((table) => `DROP TABLE ${table};`).join(" "));
  db.run("ALTER TABLE review_revision_movements DROP COLUMN items_computed_at");
  db.run("PRAGMA user_version = 21");
  assert(!db.query<{ name: string }, []>("PRAGMA table_info(review_revision_movements)").all().some((row) => row.name === "items_computed_at"), "v21 starts without the item-equivalence marker");
  const ws = legacyWorkspaceId()!;
  const user = db.query<{ id: string }, []>("SELECT id FROM users ORDER BY created_at LIMIT 1").get()!.id;
  const docs = {
    legacy: '{"kind":"legacy ReviewDoc","bytes":"v21"}',
    stage: '{"kind":"StageDoc V1","bytes":"v21"}',
    revision: '{"kind":"RevisionDoc V1","bytes":"v21"}',
    account: '{"kind":"AccountDoc V1","bytes":"v21"}',
    manifest: '{"kind":"StackManifestDoc V1","bytes":"v21"}',
    stackAccount: '{"kind":"StackAccountDoc V1","bytes":"v21"}',
  };
  db.run("INSERT INTO review_versions VALUES (?, 'v21-legacy', 1, ?, 101)", [ws, docs.legacy]);
  db.run("INSERT INTO stage_versions VALUES ('stv_0000000221', ?, 'sta_0000000221', 'v21-stage', 1, 'stg_0000000221', ?, 'stage-v21', ?, 'key_0000000221', 102)", [ws, docs.stage, user]);
  db.run("INSERT INTO review_revisions VALUES ('rvr_0000000221', ?, 'rln_0000000221', 'v21-review', 1, 'stg_0000000222', 1, ?, 'revision-v21', 103)", [ws, docs.revision]);
  db.run("INSERT INTO review_accounts VALUES ('rac_0000000221', ?, 'rln_0000000221', 'rvr_0000000221', 1, 'v21-review', 1, 1, ?, 'account-v21', ?, 'key_0000000221', 104)", [ws, docs.account, user]);
  db.run("INSERT INTO review_stacks VALUES ('rsk_0000000221', ?, 'v21-stack', 'V21 stack', 'acme/repo', 7, 'main', 'inferred', NULL, NULL, 'anonymous', NULL, NULL, NULL, 1, ?, 'key_0000000221', 105, 105)", [ws, user]);
  db.run("INSERT INTO review_stack_manifests VALUES ('rsm_0000000221', 'rsk_0000000221', ?, 'v21-stack', 1, 0, 'created', 1, ?, 'manifest-v21', 106)", [ws, docs.manifest]);
  db.run("INSERT INTO review_stack_accounts VALUES ('rsa_0000000221', 'rsk_0000000221', 'rsm_0000000221', ?, 'v21-stack', 1, 1, ?, 'stack-account-v21', ?, 'key_0000000221', 107)", [ws, docs.stackAccount, user]);
  db.run("INSERT INTO shares (id, workspace_id, kind, target, label, token_hash, created_by, created_at) VALUES ('shr_0000000221', ?, 'review_document', 'rvr_0000000221', 'v21 link', 'v21-token-hash', ?, 108)", [ws, user]);
  db.run("INSERT INTO share_document_capabilities VALUES ('shr_0000000221', ?, 'review_revision', 'rvr_0000000221', 108, 'snapshot')", [ws]);
  db.run("INSERT INTO review_threads VALUES ('rth_0000000221', ?, 'lineage', 'rln_0000000221', NULL, ?, 1, 109)", [ws, user]);
  db.run("INSERT INTO review_thread_anchors (thread_id, workspace_id, anchor_kind, lineage_id, revision_id) VALUES ('rth_0000000221', ?, 'review', 'rln_0000000221', 'rvr_0000000221')", [ws]);
  db.run("INSERT INTO review_thread_entries VALUES ('rte_0000000221', 'rth_0000000221', ?, 1, 'message', 'member', ?, NULL, NULL, NULL, 'v21 local message', 110)", [ws, user]);
  db.run("INSERT INTO review_github_threads VALUES ('rgt_0000000221', ?, 'rln_0000000221', 7, 21, 'PRRT_v21', '9007199254740993', 111)", [ws]);
  db.run("INSERT INTO review_github_thread_observations (id, workspace_id, thread_id, source_kind, source_id, source_observed_at, path, side, start_line, end_line, original_start_line, original_end_line, commit_sha, original_commit_sha, resolved, outdated, deleted, github_url, digest, observed_at) VALUES ('rgo_0000000221', ?, 'rgt_0000000221', 'webhook', 'delivery-v21', 111, 'src/value.ts', 'new', 1, 1, 1, 1, ?, ?, 0, 0, 0, 'https://github.com/acme/repo/pull/21#discussion', 'thread-v21', 111)", [ws, "a".repeat(40), "a".repeat(40)]);
  db.run("INSERT INTO review_github_comments VALUES ('rgc_0000000221', ?, 'rgt_0000000221', '9007199254740993', 'PRRC_v21', 112, 112)", [ws]);
  db.run("INSERT INTO review_github_comment_observations VALUES ('rgo_0000000222', ?, 'rgc_0000000221', 'webhook', 'delivery-v21', 112, 'octocat', 'v21 imported comment', 'https://github.com/acme/repo/pull/21#discussion', 112, 0, 'comment-v21', 112)", [ws]);
  db.run("INSERT INTO projects (id, workspace_id, slug, parent_id, title, description, status, created_at, updated_at) VALUES ('prj_0000000221', ?, 'v21-project', NULL, 'V21 project', '', 'open', 113, 113)", [ws]);
  db.run("INSERT INTO project_review_lineages VALUES ('prj_0000000221', ?, 'v21-review', 113)", [ws]);
  db.run("INSERT INTO project_review_stacks VALUES ('prj_0000000221', ?, 'v21-stack', 113)", [ws]);
  db.run(
    "INSERT INTO review_revision_movements (revision_id, workspace_id, lineage_id, previous_revision_id, unchanged, revised, new, removed, computed_at) VALUES ('rvr_0000000221', ?, 'rln_0000000221', 'rvr_0000000220', 1, 2, 3, 4, 114)",
    [ws],
  );
  const movementBefore = db.query("SELECT revision_id, workspace_id, lineage_id, previous_revision_id, unchanged, revised, new, removed, computed_at FROM review_revision_movements WHERE revision_id = 'rvr_0000000221'").get();

  const populated = [
    "review_versions", "stage_versions", "review_revisions", "review_accounts",
    "review_stacks", "review_stack_manifests", "review_stack_accounts", "shares",
    "share_document_capabilities", "review_threads", "review_thread_anchors",
    "review_thread_entries", "review_github_threads", "review_github_thread_observations",
    "review_github_comments", "review_github_comment_observations", "projects",
    "project_review_lineages", "project_review_stacks",
  ];
  const before = new Map(populated.map((table) => [table, db.query(`SELECT * FROM ${table} ORDER BY rowid`).all()]));
  const backup = db.serialize();
  migrate();
  assert((db.query("PRAGMA user_version").get() as { user_version: number }).user_version === 24, "v21 reaches v24");
  for (const table of populated) {
    assert(JSON.stringify(db.query(`SELECT * FROM ${table} ORDER BY rowid`).all()) === JSON.stringify(before.get(table)), `${table} survives v22 byte for byte`);
  }
  const movementColumns = db.query<{ name: string }, []>("PRAGMA table_info(review_revision_movements)").all().map((row) => row.name);
  assert(movementColumns.includes("items_computed_at"), "v22 adds the item-equivalence marker to v21 movement rows");
  const movementAfter = db.query("SELECT revision_id, workspace_id, lineage_id, previous_revision_id, unchanged, revised, new, removed, computed_at FROM review_revision_movements WHERE revision_id = 'rvr_0000000221'").get();
  assert(JSON.stringify(movementAfter) === JSON.stringify(movementBefore), "the v21 movement fact survives v22 byte for byte");
  assert((db.query<{ items_computed_at: number | null }, []>("SELECT items_computed_at FROM review_revision_movements WHERE revision_id = 'rvr_0000000221'").get()?.items_computed_at ?? null) === null, "the existing v21 movement awaits one item-equivalence backfill");
  for (const table of V22_TABLES) {
    assert(!!db.query("SELECT name FROM sqlite_master WHERE type = 'table' AND name = ?").get(table), `${table} exists after v22`);
    assert((db.query(`SELECT COUNT(*) AS n FROM ${table}`).get() as { n: number }).n === 0, `${table} starts empty`);
  }
  let refused = false;
  try { assertDatabaseVersionSupported(21); } catch (error) {
    refused = true;
    const message = (error as Error).message;
    assert(/user_version 24/.test(message) && /0 through 21/.test(message), `the v21 refusal names the version and range, got: ${message}`);
  }
  assert(refused, "the v21 maximum refuses v24");

  db.close();
  for (const suffix of ["", "-wal", "-shm"]) rmSync(join(dataDir, `seer.db${suffix}`), { force: true });
  writeBackup(join(dataDir, "seer.db"), backup);
  const restored = new Sqlite(join(dataDir, "seer.db"), { readonly: true });
  assert((restored.query("PRAGMA user_version").get() as { user_version: number }).user_version === 21, "restore reopens under v21");
  for (const table of populated) {
    assert(JSON.stringify(restored.query(`SELECT * FROM ${table} ORDER BY rowid`).all()) === JSON.stringify(before.get(table)), `restore recovers every ${table} row`);
  }
  for (const table of V22_TABLES) assert(restored.query("SELECT name FROM sqlite_master WHERE type = 'table' AND name = ?").get(table) === null, `${table} is absent after restore`);
  assert(!restored.query<{ name: string }, []>("PRAGMA table_info(review_revision_movements)").all().some((row) => row.name === "items_computed_at"), "restore removes the v22 movement marker");
  assert(JSON.stringify(restored.query("SELECT revision_id, workspace_id, lineage_id, previous_revision_id, unchanged, revised, new, removed, computed_at FROM review_revision_movements WHERE revision_id = 'rvr_0000000221'").get()) === JSON.stringify(movementBefore), "restore recovers the v21 movement row");
  restored.close();
  console.log("migrate v21: all assertions passed");
  process.exit(0);
}

// The v23 personal projection migration over populated local handling, conversation,
// immutable documents and credentials. Rollback restores the serialized v22 database.
if (SCENARIO === "v22") {
  process.env.AUTH_DISABLED = "true";
  delete process.env.ALLOWED_EMAILS;
  const { migrate, assertDatabaseVersionSupported } = await import("../src/migrate");
  ...
  const { rmSync, writeFileSync: writeBackup } = await import("node:fs");
  migrate();
  db.exec([...V23_TABLES, ...V24_TABLES].map((table) => `DROP TABLE ${table};`).join(" ") + V22_THREAD_OBSERVATIONS_RESTORE + " PRAGMA user_version = 22;");

  const ws = "ws_0000000000", user = "usr_0000000000";
  const head = "a".repeat(40);
  const docs = {
    legacy: '{"kind":"legacy ReviewDoc","bytes":"v22"}',
    stage: '{"kind":"StageDoc V1","bytes":"v22"}',
    revision: '{"kind":"RevisionDoc V1","bytes":"v22"}',
    account: '{"kind":"AccountDoc V1","bytes":"v22"}',
    manifest: '{"kind":"StackManifestDoc V1","bytes":"v22"}',
    stackAccount: '{"kind":"StackAccountDoc V1","bytes":"v22"}',
  };
  db.run("INSERT INTO review_versions VALUES (?, 'v22-legacy', 1, ?, 201)", [ws, docs.legacy]);
  db.run("INSERT INTO stage_versions VALUES ('stv_0000000231', ?, 'sta_0000000231', 'v22-stage', 1, 'stg_0000000231', ?, 'stage-v22', ?, 'key_0000000231', 202)", [ws, docs.stage, user]);
  db.run("INSERT INTO review_revisions VALUES ('rvr_0000000231', ?, 'rln_0000000231', 'v22-review', 1, 'stg_0000000232', 1, ?, 'revision-v22', 203)", [ws, docs.revision]);
  db.run("INSERT INTO review_accounts VALUES ('rac_0000000231', ?, 'rln_0000000231', 'rvr_0000000231', 1, 'v22-review', 1, 1, ?, 'account-v22', ?, 'key_0000000231', 204)", [ws, docs.account, user]);
  db.run("INSERT INTO review_stack_manifests VALUES ('rsm_0000000231', 'rsk_0000000231', ?, 'v22-stack', 1, 0, 'created', 1, ?, 'manifest-v22', 205)", [ws, docs.manifest]);
  db.run("INSERT INTO review_stack_accounts VALUES ('rsa_0000000231', 'rsk_0000000231', 'rsm_0000000231', ?, 'v22-stack', 1, 1, ?, 'stack-account-v22', ?, 'key_0000000231', 206)", [ws, docs.stackAccount, user]);
  db.run("INSERT INTO shares (id,workspace_id,kind,target,label,token_hash,created_by,created_at) VALUES ('shr_0000000231',?,'review_document','rvr_0000000231','v22 link','v22-hash',?,207)", [ws, user]);
  db.run("INSERT INTO review_threads VALUES ('rth_0000000231',?,'lineage','rln_0000000231',NULL,?,1,208)", [ws, user]);
  db.run("INSERT INTO review_thread_anchors (thread_id,workspace_id,anchor_kind,lineage_id,revision_id) VALUES ('rth_0000000231',?,'review','rln_0000000231','rvr_0000000231')", [ws]);
  db.run("INSERT INTO review_thread_entries VALUES ('rte_0000000231','rth_0000000231',?,1,'message','member',?,NULL,NULL,NULL,'v22 local message',209)", [ws, user]);
  db.run("INSERT INTO review_github_threads VALUES ('rgt_0000000231',?,'rln_0000000231',31,23,'PRRT_v22','9007199254740994',210)", [ws]);
  db.run("INSERT INTO review_github_comments VALUES ('rgc_0000000231',?,'rgt_0000000231','9007199254740994','PRRC_v22',211,211)", [ws]);
  db.run("INSERT INTO review_github_comment_observations VALUES ('rgo_0000000231',?,'rgc_0000000231','webhook','delivery-v22',211,'octocat','v22 imported comment','https://github.com/acme/repo/pull/23',211,0,'comment-v22',211)", [ws]);
  db.run("INSERT INTO review_revision_change_reads VALUES (?,'rvr_0000000231',?,'chg_" + "b".repeat(64) + "',212)", [ws, user]);
  db.run("INSERT INTO review_revision_acknowledgements VALUES (?,'rln_0000000231','rvr_0000000231',?,'sti_0000000231','material','identity-v22','explicit',NULL,NULL,NULL,213)", [ws, user]);
  db.run("INSERT INTO review_revision_acknowledgement_boundaries VALUES ('rvr_0000000231',?,'sti_0000000231',?,213)", [user, ws]);
  db.run("INSERT INTO review_revision_judgments VALUES ('rjd_0000000231',?,'rln_0000000231','rvr_0000000231',?,'approved','v22 verdict','ack-v22',1,214)", [ws, user]);
  db.run("INSERT INTO review_revision_judgment_items VALUES ('rjd_0000000231','sti_0000000231','material','identity-v22','explicit',NULL,NULL,NULL,213)");
  db.run("INSERT INTO review_stack_judgments VALUES ('sjd_0000000231',?,'rsk_0000000231','rsm_0000000231',?,'changes_requested','v22 stack verdict','stack-ack-v22',1,215)", [ws, user]);
  db.run("INSERT INTO review_stack_judgment_items VALUES ('sjd_0000000231','rvr_0000000231','sti_0000000231','material','identity-v22','explicit',NULL,NULL,NULL,213)");
  db.run("INSERT INTO review_lineage_prs VALUES ('rln_0000000231',?,'v22-review',31,'acme/repo',23,'feature','main','user',NULL,?,'guc_0000000231',216,NULL)", [ws, user]);
  db.run("INSERT INTO review_pr_observations VALUES ('pob_0000000231',?,'rln_0000000231',31,'acme/repo',23,'V22 review','open',0,0,'main',?,'feature',?,?,216,216,'user',NULL,?,'guc_0000000231','observation-v22')", [ws, "c".repeat(40), head, "d".repeat(40), user]);
  db.run("INSERT INTO github_user_credentials (id,user_id,kind,label,secret,account_login,account_id,scopes,expires_at,created_at,last_used_at,revoked_at,dead_at) VALUES ('guc_0000000231',?,'pat','v22 credential','sealed-v22','octocat',23,'[]',NULL,217,NULL,NULL,NULL)", [user]);
  db.run("INSERT INTO projects (id,workspace_id,slug,parent_id,title,description,status,created_at,updated_at) VALUES ('prj_0000000231',?,'v22-project',NULL,'V22 project','','open',218,218)", [ws]);
  db.run("INSERT INTO project_review_lineages VALUES ('prj_0000000231',?,'v22-review',218)", [ws]);

  const populated = [
    "review_versions", "stage_versions", "review_revisions", "review_accounts",
    "review_stack_manifests", "review_stack_accounts", "shares", "review_threads",
    "review_thread_anchors", "review_thread_entries", "review_github_threads",
    "review_github_comments", "review_github_comment_observations",
    "review_revision_change_reads", "review_revision_acknowledgements",
    "review_revision_acknowledgement_boundaries", "review_revision_judgments",
    "review_revision_judgment_items", "review_stack_judgments",
    "review_stack_judgment_items", "review_lineage_prs", "review_pr_observations",
    "github_user_credentials", "projects", "project_review_lineages",
  ];
  const before = new Map(populated.map((table) => [table, db.query(`SELECT * FROM ${table} ORDER BY rowid`).all()]));
  const backup = db.serialize();
  migrate();
  assert((db.query("PRAGMA user_version").get() as { user_version: number }).user_version === 24, "v22 reaches v24");
  for (const table of populated) {
    assert(JSON.stringify(db.query(`SELECT * FROM ${table} ORDER BY rowid`).all()) === JSON.stringify(before.get(table)), `${table} survives v24 byte for byte`);
  }
  for (const table of V23_TABLES) {
    assert(!!db.query("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table), `${table} exists after v23`);
    assert((db.query(`SELECT COUNT(*) AS n FROM ${table}`).get() as { n: number }).n === 0, `${table} starts empty`);
  }
  const ownershipPk = db.query<{ name: string; pk: number }, []>("PRAGMA table_info(review_github_viewed_ownership)").all().filter((column) => column.pk > 0).sort((a, b) => a.pk - b.pk).map((column) => column.name);
  assert(JSON.stringify(ownershipPk) === JSON.stringify(["lineage_id", "user_id", "path"]), "v23 Viewed ownership follows the PR file rather than a head");
  const substitutionColumns = new Set(db.query<{ name: string }, []>("PRAGMA table_info(review_github_viewed_credential_substitutions)").all().map((column) => column.name));
  for (const name of ["ownership_credential_id", "credential_id", "account_login", "account_id"]) assert(substitutionColumns.has(name), `v23 Viewed substitution audit includes ${name}`);
  const submissionColumns = new Set(db.query<{ name: string }, []>("PRAGMA table_info(review_github_submissions)").all().map((column) => column.name));
  for (const name of ["local_entry_id", "projection_key", "actor_generation", "mutation_started_at"]) assert(submissionColumns.has(name), `v23 submissions include ${name}`);
  for (const name of ["local_message_id", "local_event_id"]) assert(!submissionColumns.has(name), `v23 submissions remove split ${name}`);
  const threadObservationSchema = db.query<{ sql: string }, []>("SELECT sql FROM sqlite_master WHERE type='table' AND name='review_github_thread_observations'").get()!.sql;
  assert(!/UNIQUE\s*\(thread_id,\s*digest\)/i.test(threadObservationSchema), "v23 permits repeated remote resolution states");
  db.run("INSERT INTO review_github_thread_observations (id,workspace_id,thread_id,source_kind,source_id,source_observed_at,resolved,outdated,deleted,digest,observed_at) VALUES ('rgo_v23_repeat_1',?,'rgt_0000000231','webhook','repeat-1',220,1,0,0,'same-state',220)", [ws]);
  db.run("INSERT INTO review_github_thread_observations (id,workspace_id,thread_id,source_kind,source_id,source_observed_at,resolved,outdated,deleted,digest,observed_at) VALUES ('rgo_v23_repeat_2',?,'rgt_0000000231','webhook','repeat-2',221,1,0,0,'same-state',221)", [ws]);
  db.run("DELETE FROM review_github_thread_observations WHERE id IN ('rgo_v23_repeat_1','rgo_v23_repeat_2')");
  db.run("INSERT INTO review_github_projection_preferences VALUES (?, 'rln_0000000231', ?, 'guc_0000000231', 1, 219, 219)", [ws, user]);
  assert(!!db.query("SELECT 1 FROM review_github_projection_preferences WHERE lineage_id='rln_0000000231'").get(), "v23 accepts one exact personal preference");
  let refused = false;
  try { assertDatabaseVersionSupported(22); } catch (error) {
    refused = true;
    assert(/user_version 24/.test((error as Error).message) && /0 through 22/.test((error as Error).message), "the v22 maximum names the v24 refusal");
  }
  assert(refused, "a v22 binary refuses the migrated database");

  db.close();
  for (const suffix of ["", "-wal", "-shm"]) rmSync(join(dataDir, `seer.db${suffix}`), { force: true });
  writeBackup(join(dataDir, "seer.db"), backup);
  const restored = new Sqlite(join(dataDir, "seer.db"), { readonly: true });
  assert((restored.query("PRAGMA user_version").get() as { user_version: number }).user_version === 22, "restore reopens under v22");
  for (const table of populated) assert(JSON.stringify(restored.query(`SELECT * FROM ${table} ORDER BY rowid`).all()) === JSON.stringify(before.get(table)), `restore recovers every ${table} row`);
  for (const table of V23_TABLES) assert(restored.query("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table) === null, `${table} is absent after restore`);
  const restoredThreadSchema = (restored.query("SELECT sql FROM sqlite_master WHERE type='table' AND name='review_github_thread_observations'").get() as { sql: string }).sql;
  assert(/UNIQUE\s*\(thread_id,\s*digest\)/i.test(restoredThreadSchema), "restore recovers the exact v22 thread-observation constraint");
  restored.close();
  console.log("migrate v22: all assertions passed");
  process.exit(0);
}

// The v24 default-cutover migration is additive over every old and immutable document
// family. Rollback is the exact serialized v23 database; a v23 image refuses v24.
if (SCENARIO === "v23") {
  process.env.AUTH_DISABLED = "true";
  delete process.env.ALLOWED_EMAILS;
  const { migrate, assertDatabaseVersionSupported } = await import("../src/migrate");
  ...
  const { rmSync, writeFileSync: writeBackup } = await import("node:fs");
  migrate();
  db.exec(V24_TABLES.map((table) => `DROP TABLE ${table};`).join(" ") + " PRAGMA user_version = 23;");

  const ws = "ws_0000000000", user = "usr_0000000000";
  const docs = {
    legacy: '{"kind":"legacy ReviewDoc","bytes":"v23"}',
    stage: '{"kind":"StageDoc V1","bytes":"v23"}',
    revision: '{"kind":"RevisionDoc V1","bytes":"v23"}',
    account: '{"kind":"AccountDoc V1","bytes":"v23"}',
    manifest: '{"kind":"StackManifestDoc V1","bytes":"v23"}',
    stackAccount: '{"kind":"StackAccountDoc V1","bytes":"v23"}',
  };
  db.run("INSERT INTO reviews VALUES (?, 'v23-legacy', 1, 301)", [ws]);
  db.run("INSERT INTO review_versions VALUES (?, 'v23-legacy', 1, ?, 301)", [ws, docs.legacy]);
  db.run("INSERT INTO review_annotations VALUES ('ann_0000000240', ?, 'v23-legacy', 'summary', 'summary', NULL, 'v23 question', 'open', NULL, 1, 302)", [ws]);
  db.run("INSERT INTO review_reads VALUES (?, 'v23-legacy', ?, 1, 303)", [ws, user]);
  db.run("INSERT INTO stage_versions VALUES ('stv_0000000240', ?, 'sta_0000000240', 'v23-stage', 1, 'stg_0000000240', ?, 'stage-v23', ?, 'key_0000000240', 304)", [ws, docs.stage, user]);
  db.run("INSERT INTO review_revisions VALUES ('rvr_0000000240', ?, 'rln_0000000240', 'v23-review', 1, 'stg_0000000241', 1, ?, 'revision-v23', 305)", [ws, docs.revision]);
  db.run("INSERT INTO review_accounts VALUES ('rac_0000000240', ?, 'rln_0000000240', 'rvr_0000000240', 1, 'v23-review', 1, 1, ?, 'account-v23', ?, 'key_0000000240', 306)", [ws, docs.account, user]);
  db.run("INSERT INTO review_stack_manifests VALUES ('rsm_0000000240', 'rsk_0000000240', ?, 'v23-stack', 1, 0, 'created', 1, ?, 'manifest-v23', 307)", [ws, docs.manifest]);
  db.run("INSERT INTO review_stack_accounts VALUES ('rsa_0000000240', 'rsk_0000000240', 'rsm_0000000240', ?, 'v23-stack', 1, 1, ?, 'stack-account-v23', ?, 'key_0000000240', 308)", [ws, docs.stackAccount, user]);
  db.run("INSERT INTO shares (id, workspace_id, kind, target, label, token_hash, created_by, created_at) VALUES ('shr_0000000240', ?, 'review', 'v23-legacy', 'v23 link', 'v23-link-hash', ?, 309)", [ws, user]);
  db.run("INSERT INTO review_threads VALUES ('rth_0000000240', ?, 'lineage', 'rln_0000000240', NULL, ?, 1, 310)", [ws, user]);
  db.run("INSERT INTO review_revision_change_reads VALUES (?, 'rvr_0000000240', ?, 'chg_" + "d".repeat(64) + "', 311)", [ws, user]);
  db.run("INSERT INTO review_github_projection_preferences VALUES (?, 'rln_0000000240', ?, 'guc_0000000240', 1, 312, 312)", [ws, user]);

  const populated = [
    "reviews", "review_versions", "review_annotations", "review_reads", "stage_versions",
    "review_revisions", "review_accounts", "review_stack_manifests", "review_stack_accounts",
    "shares", "review_threads", "review_revision_change_reads", "review_github_projection_preferences",
  ];
  const before = new Map(populated.map((table) => [table, db.query(`SELECT * FROM ${table} ORDER BY rowid`).all()]));
  const backup = db.serialize();

  migrate();
  assert((db.query("PRAGMA user_version").get() as { user_version: number }).user_version === 24, "v23 reaches v24");
  for (const table of populated) {
    assert(JSON.stringify(db.query(`SELECT * FROM ${table} ORDER BY rowid`).all()) === JSON.stringify(before.get(table)), `${table} survives v24 byte for byte`);
  }
  for (const table of V24_TABLES) {
    assert(!!db.query("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table), `${table} exists after v24`);
    assert((db.query(`SELECT COUNT(*) AS n FROM ${table}`).get() as { n: number }).n === 0, `${table} starts empty`);
  }
  let refused = false;
  try { assertDatabaseVersionSupported(23); } catch (error) {
    refused = true;
    assert(/user_version 24/.test((error as Error).message) && /0 through 23/.test((error as Error).message), "the v23 maximum names the v24 refusal");
  }
  assert(refused, "a v23 binary refuses the migrated database");

  db.close();
  for (const suffix of ["", "-wal", "-shm"]) rmSync(join(dataDir, `seer.db${suffix}`), { force: true });
  writeBackup(join(dataDir, "seer.db"), backup);
  const restored = new Sqlite(join(dataDir, "seer.db"), { readonly: true });
  assert((restored.query("PRAGMA user_version").get() as { user_version: number }).user_version === 23, "restore reopens under v23");
  for (const table of populated) {
    assert(JSON.stringify(restored.query(`SELECT * FROM ${table} ORDER BY rowid`).all()) === JSON.stringify(before.get(table)), `restore recovers every ${table} row`);
  }
  for (const table of V24_TABLES) {
    assert(restored.query("SELECT name FROM sqlite_master WHERE type='table' AND name=?").get(table) === null, `${table} is absent after restore`);
  }
  restored.close();
  console.log("migrate v23: all assertions passed");
  process.exit(0);
}
...
if (SCENARIO === "newer") {
  ...
  try {
    ...
    threw = true;
    const message = (err as Error).message;
    assert(/user_version 25/.test(message) && /0 through 24/.test(message), `the refusal names the version and the range, got: ${message}`);
  }
  assert(threw, "a user_version above the ladder's top must be refused");
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 25, `the refused database is left untouched, got ${uv}`);
