...
if (SCENARIO === "v0") {
  ...
  assert(uv === 9, `user_version should be 9, got ${uv}`);
...
if (SCENARIO === "v2") {
  ...
  assert(uv2 === 9, `user_version stays 9 after re-run, got ${uv2}`);
  ...
  try {
    ...
    threw = true;
    assert(/user_version 10/.test((err as Error).message), `actionable message, got: ${(err as Error).message}`);
  }
...
if (SCENARIO === "v5stops") {
  ...
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 9, `ordinary boot reaches 9, got ${uv}`);
  const still = db
...
if (SCENARIO === "v5dropafterboot") {
  ...
  assert(first === 9, `ordinary boot reaches 9, got ${first}`);
  assert(
    !!db
      .query("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'review_freshness'")
      .get(),
  ...
  assert(after === 9, `and the version is untouched by it, got ${after}`);
