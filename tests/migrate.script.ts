...
if (SCENARIO === "v0") {
  ...
  assert(uv === 14, `user_version should be 14, got ${uv}`);
...
if (SCENARIO === "v11") {
  ...
  migrate();
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 14, `v11 database reaches 14, got ${uv}`);
...
if (SCENARIO === "v12") {
  ...
  migrate();
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 14, `v12 database reaches 14, got ${uv}`);
...
if (SCENARIO === "fresh") {
  ...
  for (const table of [...V3_TABLES, ...V4_TABLES, ...V5_TABLES, ...V6_TABLES, ...V12_TABLES, ...V13_TABLES, ...V14_TABLES]) {
...
if (SCENARIO === "v2") {
  ...
  assert(uv2 === 14, `user_version stays 14 after re-run, got ${uv2}`);
  ...
  try {
    ...
    threw = true;
    assert(/user_version 15/.test((err as Error).message), `actionable message, got: ${(err as Error).message}`);
  }
...
if (SCENARIO === "v5stops") {
  ...
  const uv = (db.query("PRAGMA user_version").get() as { user_version: number }).user_version;
  assert(uv === 14, `ordinary boot reaches 14, got ${uv}`);
  const still = db
...
if (SCENARIO === "v5dropafterboot") {
  ...
  assert(first === 14, `ordinary boot reaches 14, got ${first}`);
  assert(
    !!db
      .query("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'review_freshness'")
      .get(),
  ...
  assert(after === 14, `and the version is untouched by it, got ${after}`);
