...
- **Share links.** A bundle can also be served under `/s/<token>/`, from a revocable link a member mints on the bundle's row in `/bundles`. The whole tree comes with it — assets, subdirectories, and `/s/<token>/v/N/` to pin a version — so a bundle in a **private** workspace can be handed to someone with no account at all, and taken back later. The live-reload socket on a shared page carries `?share=<token>` instead of the workspace, so the workspace and slug are read off the share row rather than off the query. An unknown, revoked, or expired token is the same generic 404 as one that never existed. See `docs/shares.md`.
