...
- **Share links.** A bundle can also be served under `/s/<token>/`, from a revocable link minted on the bundle's row in `/bundles` or by an agent holding a workspace API key (`POST /api/shares`). The whole tree comes with it — assets, subdirectories, and `/s/<token>/v/N/` to pin a version — so a bundle in a **private** workspace can be handed to someone with no account at all, and taken back later. Everything a share serves is `private, no-cache` and `X-Robots-Tag: noindex, nofollow`, and the served page declares no `og:url`: the token is the URL, so no intermediary caches it, no crawler indexes it, and no unfurler republishes it. The live-reload socket carries `?share=<token>` instead of the workspace, so the workspace and slug are read off the share row rather than off the query, and revoking a share closes the socket it had already opened. An unknown, revoked, or expired token is the same generic 404 as one that never existed. See `docs/shares.md`.
